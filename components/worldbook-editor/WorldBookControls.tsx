        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 sm:gap-3">
          <div className="flex items-center space-x-1.5 sm:space-x-2 flex-wrap">
            ...
            <button
              ...
              disabled={!canBulk}
              className={`px-2 sm:px-3 py-1 sm:py-1.5 rounded-md border border-ink text-cream-soft text-2xs sm:text-xs transition-colors ${!canBulk ? "opacity-50 cursor-not-allowed" : "hover:border-amber-400 hover:text-amber-300"}`}
            >
  ...
  return (
    <div className="flex items-center gap-1 sm:gap-2">
      <div className="flex items-center gap-1 sm:gap-1.5">
        <span className={`text-2xs sm:text-xs text-ink-soft font-medium ${serifFontClass}`}>{label}</span>
      </div>
